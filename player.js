...
document.querySelectorAll('.play[data-dz],.play[data-url]').forEach(btn=>{
  btn.addEventListener('click',e=>{
    ...
    const {dz,url,title,artist}=btn.dataset;
    if(dz){ // stored Deezer preview URLs expire — fetch a fresh one via JSONP
      const cb='c'+dz;
      window._dz[cb]=d=>{if(d&&d.preview)start(btn,d.preview,title,artist);
    ...
    }else if(url){start(btn,url,title,artist);}
  });
});
